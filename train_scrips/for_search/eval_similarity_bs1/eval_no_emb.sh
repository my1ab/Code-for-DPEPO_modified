# =============================================================================
# 相似度方案对比训练 3/3：不使用 emb（精确匹配）
#
# 用途：评估关闭嵌入相似度（精确匹配）时的 penalty 效果
# 预期：penalty 对有效动作几乎完全失效（捕获率仅 ~2.7%），
#       退化为仅能捕获完全字符串相等的重复（如 null vs null、卡死循环）
#
# 对比组：
#   - eval_emb_0.9.sh：emb + threshold=0.9（保守惩罚）
#   - eval_emb_0.8.sh：emb + threshold=0.8（激进惩罚）
#   - 本脚本：不使用 emb（精确匹配，penalty 基本失效）
#
# 训练完成后 checkpoint 保存到：2gpu_eval_no_emb/
# 日志保存到：2gpu_eval_no_emb/log/
# =============================================================================
...
# ========== 输出路径（本脚本独占） ==========
ckpt_dir=${_CODE_BASE}/2gpu_emb_search_noemb
LOG_FILE="$ckpt_dir/log/search_noemb_$(date +%Y%m%d_%H%M%S).log"
...
nohup python3 train_scrips/for_search/main_ppo_search.py \
    ...
    trainer.experiment_name='grpo_1.5b_search_noemb' \
