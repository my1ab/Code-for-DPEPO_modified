...
# ========== 输出路径（本脚本独占） =========
ckpt_dir=${_CODE_BASE}/3emb_model_bs4/search_no_emb_bs4_lr1e-4
LOG_FILE="$ckpt_dir/log/search_no_emb_bs4_$(date +%Y%m%d_%H%M%S).log"
...
kl_loss_coef=0.001
decay=0
# ppo_epochs: 每个 rollout batch 的梯度更新次数（内层循环）
#   - ppo_epochs=1（yaml 默认）: 每步 rollout 数据只更新 1 次梯度
#   - ppo_epochs=2（当前设置）: 同一批数据更新 2 次，对 40% 有效 group 做二次利用
#   - 不增加 rollout 时间（最耗时部分），只增加 actor 梯度更新时间
#   - 注意：critic.ppo_epochs 自动继承 actor 的值（yaml 中 ${actor_rollout_ref.actor.ppo_epochs}）
#   - 与 trainer.total_epochs 不同：total_epochs 控制外层遍历数据集的轮数
ppo_epochs=1

# ===================== 激进策略说明（rank=32, ppo_epochs=1）=====================
# 目标: 在 rank=32 + ppo_epochs=1 约束下，激进放大梯度更新效果
# 基线: bs2_noemb (lr=1e-6, kl=0.01, ppo_epochs=1) -> grad_norm=0.082, success=41.08%
#
# 已启用的激进手段:
#   1. lr=3e-5（原1e-6的30倍，原1e-5的3倍）-> 等效更新量级 3e-5×0.082=2.46e-6（原1e-6的3倍）
#   2. kl_loss_coef=0.001（原0.01的1/10）-> 释放被 KL 对冲的梯度方向
#   3. weight_decay=0.0（原0.01->0）-> 停止对 LoRA 参数的权重衰减，保留全部梯度
#   注: clip_ratio 保持默认 0.2（pg_clipfrac 仅 0.075%，远未触及阈值）
#   注: grad_clip 保持默认 1.0（grad_norm max=0.191，远低于阈值，从未触发）
#
# 预期效果:
#   - grad_norm 不变（~0.082，由 rank=32 决定）
#   - 有效更新量级: 3e-5 × 0.082 = 2.46e-6（vs 原始 8.2e-8，提升 30 倍）
#   - 风险: 策略可能偏离 SFT，通过 save_freq=10 和 KL loss 监控
...
nohup python3 train_scrips/for_search/main_ppo_search.py \
    ...
    trainer.experiment_name='grpo_1.5b_search_emb080' \
    ...
    actor_rollout_ref.actor.optim.weight_decay=$decay \
