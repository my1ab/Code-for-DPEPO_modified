# =============================================================================
# 相似度方案对比训练 1/3：emb + threshold=0.9
#
# 用途：评估使用 bge-large-en 嵌入模型 + 阈值 0.9 时的 penalty 效果
# 预期：阈值偏高，惩罚力度弱，接近退化，但仍能捕获 raw>0.9 的强同义对
#
# 对比组：
#   - 本脚本：emb + threshold=0.9（保守惩罚）
#   - eval_emb_0.8.sh：emb + threshold=0.8（激进惩罚）
#   - eval_no_emb.sh：不使用 emb（精确匹配，penalty 基本失效）
#
# 训练完成后 checkpoint 保存到：2gpu_eval_emb_090/
# 日志保存到：2gpu_eval_emb_090/log/
# =============================================================================
...
# ========== 输出路径（本脚本独占） ==========
ckpt_dir=${_CODE_BASE}/2gpu_emb_search_090
LOG_FILE="$ckpt_dir/log/search_emb090_$(date +%Y%m%d_%H%M%S).log"
...
nohup python3 train_scrips/for_search/main_ppo_search.py \
    ...
    trainer.experiment_name='grpo_1.5b_search_emb090' \
