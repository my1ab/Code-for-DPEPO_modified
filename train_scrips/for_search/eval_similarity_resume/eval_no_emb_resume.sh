# =============================================================================
# 相似度方案对比续训 3/3：不使用 emb（精确匹配）续训
#
# 用途：从 step 500 的 merged checkpoint 续训，使用新的 500 个困难样本
# 预期：累积更多有效梯度 step，验证续训能否涨点
#
# 对比组：
#   - eval_emb_0.9_resume.sh：emb + threshold=0.9（保守惩罚）续训
#   - eval_emb_0.8_resume.sh：emb + threshold=0.8（激进惩罚）续训
#   - 本脚本：不使用 emb（精确匹配）续训
#
# 训练完成后 checkpoint 保存到：3emb_model_resume_noemb/
# 日志保存到：3emb_model_resume_noemb/log/
# =============================================================================
...
# ========== 输出路径（本脚本独占） ==========
ckpt_dir=${_CODE_BASE}/3emb_model_resume/noemb
LOG_FILE="$ckpt_dir/log/search_noemb_resume_$(date +%Y%m%d_%H%M%S).log"
...
custom_use_embedding=false     # 关闭嵌入相似度，使用精确匹配
custom_emb_threshold=0.893      # emb 关闭时此值不生效


nohup python3 train_scrips/for_search/main_ppo_search.py \
    algorithm.adv_estimator=grpo \
    env.env_path=${_CODE_BASE}/data_pipelines/gamefiles/search/search_train_tasks_resume.json \
    data.train_files=${_CODE_BASE}/data_pipelines/verl_train_data/search/search_train_resume.parquet \
    data.val_files=${_CODE_BASE}/data_pipelines/verl_train_data/search/search_test.parquet \
    actor_rollout_ref.model.path=/diskpool/home/xuxz/Code-for-DPEPO/2gpu_emb_search_noemb/global_step_500/merged \
    trainer.default_local_dir=$ckpt_dir \
    trainer.experiment_name='grpo_1.5b_search_noemb_resume' \
...
echo "Similarity config: emb=$custom_use_embedding, threshold=$custom_emb_threshold (emb OFF, threshold ignored)"
echo "Resume from: 2gpu_emb_search_noemb/global_step_500/merged"
echo "Train data: search_train_tasks_resume.json (500 new hard samples)"
