# =============================================================================
# 相似度方案对比续训 2/3：emb + threshold=0.8（续训）
#
...
#   - 本脚本：emb + threshold=0.8（激进惩罚）续训
#   - eval_no_emb_resume.sh：不使用 emb（精确匹配）续训
#
# 训练完成后 checkpoint 保存到：3emb_model_resume_080/
# 日志保存到：3emb_model_resume_080/log/
# =============================================================================
...
# ========== 输出路径（本脚本独占） ==========
ckpt_dir=${_CODE_BASE}/3emb_model_resume/emb_0.8
LOG_FILE="$ckpt_dir/log/search_emb0.8_resume_$(date +%Y%m%d_%H%M%S).log"
...
nohup python3 train_scrips/for_search/main_ppo_search.py \
    ...
    actor_rollout_ref.model.path=/diskpool/home/xuxz/Code-for-DPEPO/3emb_model_bs1/search_emb_b080_bs1/global_step_500/merged \
    trainer.default_local_dir=$ckpt_dir \
    trainer.experiment_name='grpo_1.5b_search_emb080_resume' \
...
echo "Resume from: 2gpu_emb_search_080/global_step_500/merged"
