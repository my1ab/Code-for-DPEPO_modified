# =============================================================================
# 相似度方案对比续训 1/3：emb + threshold=0.9（续训）
#
# 用途：从 step 500 的 merged checkpoint 续训，使用新的 500 个困难样本
# 预期：累积更多有效梯度 step，验证续训能否涨点
#
# 对比组：
#   - 本脚本：emb + threshold=0.9（保守惩罚）续训
#   - eval_emb_0.8_resume.sh：emb + threshold=0.8（激进惩罚）续训
#   - eval_no_emb_resume.sh：不使用 emb（精确匹配）续训
#
# 训练完成后 checkpoint 保存到：3emb_model_resume_090/
# 日志保存到：3emb_model_resume_090/log/
# =============================================================================
...
# ========== 输出路径（本脚本独占） ==========
ckpt_dir=${_CODE_BASE}/3emb_model_resume/emb_090
LOG_FILE="$ckpt_dir/log/search_emb090_resume_$(date +%Y%m%d_%H%M%S).log"
...
# custom_emb_threshold=0.9     # 阈值 0.9（保守）
custom_emb_threshold=0.893 
...
nohup python3 train_scrips/for_search/main_ppo_search.py \
    ...
    actor_rollout_ref.model.path=/diskpool/home/xuxz/Code-for-DPEPO/2gpu_emb_search_090/global_step_500/merged \
    trainer.default_local_dir=$ckpt_dir \
    trainer.experiment_name='grpo_1.5b_search_emb090_resume' \
...
echo "Resume from: 2gpu_emb_search_090/global_step_500/merged"
