...
BASE_MODEL="/diskpool/home/xuxz/ms-swift/checkpoint/Qwen2.5-1.5B-Instruct-Parallel-Epoch5-hislen8/v0-20260602-201729/checkpoint-8800"

# LoRA adapter 目录 (含 adapter_config.json + adapter_model.safetensors)
ADAPTER_DIR="/diskpool/home/xuxz/Code-for-DPEPO/2gpu_only_penalty/global_step_8/actor/lora_adapter"

# 合并后输出目录 (兼容 load_local_model 的 AutoModelForCausalLM.from_pretrained 读取)
OUTPUT_DIR="/diskpool/home/xuxz/Code-for-DPEPO/2gpu_only_penalty/global_step_8/merged"
...
if [ ! -x "$SWIFT_BIN" ]; then
    echo "[ERROR] swift not found or not executable: $SWIFT_BIN"
    exit 1
fi
...
# for f in tokenizer_config.json tokenizer.json vocab.json merges.txt chat_template.jinja; do
