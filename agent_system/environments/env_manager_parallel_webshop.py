...
class Env:
    def __init__(self, game_file):
        ...
        self.env = env
        # start_obv 已在 build_env 内部设置为原始 obs（不经过 format_obs 截断）
    ...
    def step(self, action):
        ...
        # 保存当前步的原始 obs，与 coldstart 使用的原始 obs 对齐
        self.current_obs = obs
        # format_obs 仅用于 history 记录（last_command 中），不作为 prompt 内容
        # 过滤Instruction: [SEP] Find me high quality hair loss products with seed oil for hair growth, hair loss with size: 10ml, and price lower than 50.00 dollars [SEP]
        ...
        # step 返回原始 obs（不是 format_obs 后的截断文本），与 coldstart 对齐
    ...
    def build_env(self, gamefile, env_kwargs): # the gamefile is actually seed
        ...
        infos = dict(infos or {})
        self.task = self.extract_task(obs) 
        # 与 coldstart_para_his_test_1.5B_hislen8_epoch3.5_v2.py 保持一致：
        # start_obv 使用原始 obs（不经过 format_obs 截断）
        # 任务信息由 observation 文本自身承载
        self.start_obv = obs  # 直接保存原始 obs 作为 start_obv
        formated_obs = self.format_obs(obs, add_task=False) 
...
class ParallelWebShopWorker:
    ...
    def __init__(self, game_files, num_parallel): 
        print(f"[DEBUG] ParallelWebShopWorker initialized with num_parallel={num_parallel} in ParallelWebShopWorker")
        self.env_pools = {} 
        ...
        # 标记 num_parallel会创建多个环境 而group_n只会创建副本
    ...
    def step(self, action_dict):
        ...
        # ========== 修改后的逻辑：只处理实际存在的环境，避免填充无效数据 ==========
        # 遍历所有实际存在的环境（env_pools的key从1开始）
        # for env_idx in range(1, len(self.env_pools) + 1):
        #     if env_idx in action_dict:
        #         action = action_dict[env_idx]
        #         sub_env = self.env_pools[env_idx] 
                
        #         ob,reward,done,info = sub_env.step(action) 

        #         self.last_action_manager[env_idx] = action 
        #         self.last_obs_manager[env_idx] = ob
        #         self.last_poa_manager[env_idx] = info['admissible_commands']
                
        #         admissible_commands = ','.join(info['admissible_commands']) 

        #         obs_prompt += f'<observation_{env_idx}>\nThe observation and next candidated actions of {env_idx}-th environment are:\nObservation:\n{ob}\nNext Possible Actions:\n{admissible_commands}\n</observation_{env_idx}>\n'
        ...
        for action_index,action in action_dict.items():
            if action_index <= len(self.env_pools):
                ...
                print(f'[DEBUG] oob: {oob}')
                dummy_obs = oob 
                dummy_poa = {'admissible_commands': oob}
                obs.append(dummy_obs) 
                scores.append(0) 
                dones.append(False) 
                infos.append(dummy_poa)
...
class ParallelWebShopEnvs(gym.Env):
    def __init__(self, 
        ...
        # 
        print(f"[DEBUG] ParallelWebShopEnvs initialized with num_parallel={num_parallel}, group_n={group_n} in ParallelWebShopEnvs")
        self.multi_modal = False
        self.group_n = group_n
        # 标记  self.num_parallel已弃用
        ...
        for game_file in tqdm(game_files): 
            # game_file for webshop is the seed (integer), used directly as key
            self.workers_dict[game_file] = []
            # 标记 为每个gamefile创建group_n个独立的worker
    ...
    def step(self, actions):
        ...
        return obs_list, scores_list, dones_list, infos_list, obs_prompt_list
