...
class Env:
    ...
    def reset(self):
        ...
        copied_infos['task'] = self.task  # 存储 task 字段，与 coldstart 的 info.get('goal') 对齐
...
class ParallelWebShopWorker:
    ...
    def show_basis_infos(self):
        return self.start_obv, self.admissible_commands, self.task
...
class ParallelWebShopEnvs(gym.Env):
    ...
    def get_start_info_group(self,gourped_samples):
        ...
        tasks = [elem[2] for elem in results]  # 获取 task 字段，与 coldstart 的 info.get('goal') 对齐
        
        return obvs, possible_actions, tasks 
