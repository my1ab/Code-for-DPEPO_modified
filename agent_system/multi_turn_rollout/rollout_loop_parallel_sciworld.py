...
class TrajectoryCollectorParallelSciWorld:
    ...
    def preprocess_single_sample(
        ...
        if step == 0: 
            ...
            for last_env_idx,action in last_action.items():
                ...
                poa_cur_env = last_possible_actions[last_env_idx]
    ...
    def vanilla_multi_turn_loop(
        ...
        for _step in tqdm(range(self.config.env.max_steps)): 
            ...
            for sample, cur_history_actions in zip(dict_grouped_output,history_actions):
                ...
                if len(action_dict) != 0:
                    W = self.calculate_invalid_actions_penalties(
                        history_actions=cur_history_actions,
                        action_dict=action_dict,
                        observations=observations,
                    ) 
    ...
    def calculate_penalties(self,history_actions,action_dict,observations):
        ...
        invalid_penalty = self.config.reward_model.invalid_penalty
    ...
    def calculate_invalid_actions_penalties(self,history_actions,action_dict,observations):
        COUNT_invalid_actions = 0
        for sub_obs in observations:
            if 'No known action matches that input.' in sub_obs:
                COUNT_invalid_actions += 1 
