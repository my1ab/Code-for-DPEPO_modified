...
# ── 嵌入相似度全局开关（对齐 rollout_loop_parallel_webshop.py） ──
# False: 使用原始精确匹配（默认向后兼容）
# True: 使用 bge-large-en-v1.5 嵌入模型 + 加权计数（方案 B + E，task_type='search'）
# 这里的模块级默认值可通过 config.custom.use_embedding / config.custom.emb_threshold 覆盖
# （在 TrajectoryCollectorParallelSearch.__init__ 中读取并更新全局变量）
...
EMBEDDING_SIMILARITY_THRESHOLD = 0.893
...
class TrajectoryCollectorParallelSearch:
    ...
    def __init__(self, config, tokenizer: PreTrainedTokenizer, processor=None):
        ...
        # 对齐 rollout_loop_parallel_webshop.py 的 _ensure_embedding_manager 模式
    ...
    def _ensure_embedding_manager(self):
        ...
        使用 task_type='search' 启用 Search 任务分类型策略：
          search/answer 词集匹配+嵌入兜底（方案 B+E），无 click 分支
        """
        if not self._embedding_initialized:
            self._embedding_initialized = True
            if USE_EMBEDDING_SIMILARITY:
                print(f'[INFO] [Task {GLOBAL_TASK_COUNTER}] 初始化嵌入模型，模型路径: {EMBEDDING_MODEL_PATH}，阈值: {EMBEDDING_SIMILARITY_THRESHOLD}，设备: {EMBEDDING_DEVICE}，task_type=search')
    ...
    def _calculate_width_repeat_weighted(self, action_dict, embedding_manager=None):
        ...
        calculate_transition_repeat 保持一致风格：
          - 开启嵌入相似度且 embedding_manager 可用：贪心聚类，每对相似动作贡献 min(sim_a, sim_b)
            （Search 任务过滤 null，不过滤 'look'，因 Search 无 'look' 动作）
          - 否则：精确匹配去重计数（与原副本逻辑完全一致，过滤 'look'）
        """
        global USE_EMBEDDING_SIMILARITY

        if USE_EMBEDDING_SIMILARITY and embedding_manager is not None:
            # 加权分支：过滤 null（无效动作不参与语义比较，Search 任务无 'look' 动作）
            ...
            # Search 任务无 'look' 动作，此过滤实际不生效，但保留以与副本逻辑完全一致
    ...
    def _filter_group_data_search(
        ...
        """
        Search 版 filter_group_data：丢弃组内 reward 全相同的组（全成功/全失败）。
        适配 Search 的 7 元组返回值（success_flags: np.ndarray, status_msgs: List）。
        """
        if last_try:
            return batch_list, episode_rewards, episode_lengths, traj_uid, tool_callings, success_flags, status_msgs
        ...
        if len(keep_indices) == 0:
            print("[filter_groups] 所有组都被过滤，保留全部（last_try 兜底）")
            return batch_list, episode_rewards, episode_lengths, traj_uid, tool_callings, success_flags, status_msgs
        ...
        success_flags = success_flags[keep_indices]
        status_msgs = [status_msgs[i] for i in keep_indices]
    ...
    def dynamic_multi_turn_loop(
        ...
        """
        动态采样循环（DAPO filter_groups 机制）：
        反复执行 vanilla_multi_turn_loop，丢弃组内 reward 全相同的组，
        直到收集到足够多的有效组或达到 max_try_count 上限。
        """
        total_batch_list = []
        ...
        while len(total_batch_list) < target_num and try_count < max_try_count:
            if len(total_batch_list) > 0:
                print(f"[filter_groups] valid num={len(total_batch_list)} < target={target_num}. Keep generating... ({try_count}/{max_try_count})")
            try_count += 1

            batch_list, episode_rewards, episode_lengths, traj_uid, tool_callings, success_flags, status_msgs = \
            ...
            batch_list, episode_rewards, episode_lengths, traj_uid, tool_callings, success_flags, status_msgs = \
                self._filter_group_data_search(
            ...
            total_success_flags.append(success_flags)
            total_status_msgs.append(status_msgs)

        total_episode_rewards = np.concatenate(total_episode_rewards, axis=0)
        total_episode_lengths = np.concatenate(total_episode_lengths, axis=0)
        total_traj_uid = np.concatenate(total_traj_uid, axis=0)
        total_tool_callings = np.concatenate(total_tool_callings, axis=0)
        total_success_flags = np.concatenate(total_success_flags, axis=0)
        total_status_msgs = [msg for batch_msgs in total_status_msgs for msg in batch_msgs]

        # 截断到 target_num（可能因 last_try 超过）
        if len(total_batch_list) > target_num:
            total_batch_list = total_batch_list[:target_num]
            total_episode_rewards = total_episode_rewards[:target_num]
            total_episode_lengths = total_episode_lengths[:target_num]
            total_traj_uid = total_traj_uid[:target_num]
            total_tool_callings = total_tool_callings[:target_num]
            total_success_flags = total_success_flags[:target_num]
            total_status_msgs = total_status_msgs[:target_num]

        print(f"[filter_groups] 最终收集 {len(total_batch_list)}/{target_num} 条有效轨迹 (try_count={try_count})")

        return total_batch_list, total_episode_rewards, total_episode_lengths, total_traj_uid, total_tool_callings, total_success_flags, total_status_msgs
    ...
    def multi_turn_loop(
        ...
        支持 filter_groups 动态采样（DAPO 机制）。
        """
        # 移除重复的repeat逻辑，统一由vanilla_multi_turn_loop内的group_n逻辑处理batch扩展，避免重复repeat导致batch_size加倍

        if is_train and self.config.algorithm.filter_groups.enable:
            # 动态采样（DAPO filter_groups）
