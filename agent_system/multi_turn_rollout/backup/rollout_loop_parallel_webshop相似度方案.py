...
# ── 嵌入相似度全局开关 ──
# False: 使用原始精确匹配（默认，向后兼容）
# True: 使用 bge-large-en-v1.5 嵌入模型 + 加权计数（方案 B + E）
...
# 未调用
def set_embedding_similarity(enabled=False, model_path=None, threshold=None,
                             gpu_mem_util=None):
    """运行时配置嵌入相似度开关和参数。

    在训练脚本或入口处调用:
        from agent_system.multi_turn_rollout.rollout_loop_parallel_webshop import set_embedding_similarity
        set_embedding_similarity(True)
    """
    global USE_EMBEDDING_SIMILARITY, EMBEDDING_MODEL_PATH
    global EMBEDDING_SIMILARITY_THRESHOLD, EMBEDDING_GPU_MEMORY_UTILIZATION
    USE_EMBEDDING_SIMILARITY = enabled
    if model_path is not None:
        EMBEDDING_MODEL_PATH = model_path
    if threshold is not None:
        EMBEDDING_SIMILARITY_THRESHOLD = threshold
    if gpu_mem_util is not None:
        EMBEDDING_GPU_MEMORY_UTILIZATION = gpu_mem_util
...
class TrajectoryCollectorParallelWebShop:
    ...
    def _ensure_embedding_manager(self):
        ...
        if not self._embedding_initialized:
            ...
            if USE_EMBEDDING_SIMILARITY:
                print(f'[INFO] [Task {GLOBAL_TASK_COUNTER}] 初始化嵌入模型，模型路径: {EMBEDDING_MODEL_PATH}，阈值: {EMBEDDING_SIMILARITY_THRESHOLD}')
                ...
                self.embedding_manager = None
        # print(f' 完成EmbeddingManager初始化测试  正在退出程序...')
        # os._exit(0)
    ...
    def _calculate_width_repeat_weighted(self, action_dict, embedding_manager=None):
        ...
        calculate_transition_repeat 保持一致风格：
          ...
          - 否则：精确匹配去重计数（与原逻辑完全一致，只过滤 'look'，不过滤 'null'）
        """
        global USE_EMBEDDING_SIMILARITY

        if USE_EMBEDDING_SIMILARITY and embedding_manager is not None:
            # 加权分支：过滤 look 和 null（无效动作不参与语义比较）
            actions = [v for v in action_dict.values() if v != 'look' and v != 'null']
            n = len(actions)
            if n <= 1:
                return 0
            repeat_count = 0.0
            ...
            # 精确匹配分支：与原逻辑完全一致，只过滤 look，不过滤 null
            # 原逻辑: actions_wo_look = [elem for elem in action_dict.values() if elem != 'look']
            #          COUNT_width_repeat = len(actions_wo_look) - len(set(actions_wo_look))
            actions = [v for v in action_dict.values() if v != 'look']
