...
class TrajectoryCollectorParallelWebShop:
    ...
    def vanilla_multi_turn_loop(
        ...
        print(f'[DEBUG] group_n = {group_n} in rollout loop, is_train={is_train}')
        ...
        for _step in tqdm(range(self.config.env.max_steps)):
            ...
            for bs in range(batch_size):
                ...
                if any_done:
                    ...
                    # ===== 锁定维度的正确转换逻辑 =====
                    # dones结构: [batch_size][num_parallel] → 5×5 或 5×1
                    # bs范围: 0~batch_size-1 (batch_size = train_batch_size × group_n)
                    # task_idx = bs // group_n → 原始任务索引（0~train_batch_size-1）
                    # worker_id_in_task = bs % group_n → 当前任务组内的worker索引（0~group_n-1）
                    # 从当前worker的dones列表中找到实际完成的环境索引
                    # dones[bs]
                    # for env_idx, done in enumerate(dones[bs]):
                    #     if done:
                    #         completed_env_in_worker = env_idx
                    #         break
                    ...
                    # 如果同一个task_idx组内的任意一个任务完成，将该组内所有5个并行任务都标记为完成
                    # 这样同一个原始任务的所有并行采样都会提前结束，不需要等待所有任务都完成
                    start_bs = task_idx * group_n
                    end_bs = start_bs + group_n
                    for g_bs in range(start_bs, min(end_bs, batch_size)):
                        if not is_done[g_bs]:
                            is_done[g_bs] = True
                            turn_in_range[g_bs] = False
                elif null_count[bs] >= 2:
                    # 连续2步所有动作都是null，提前退出，同样标记整个组内的所有任务
                    task_idx = bs // group_n
                    status_msgs[bs] = f"Task {task_idx} exit(all null) at turn {_step + 1}"
                    print(status_msgs[bs])
                    # 将该task_idx组内的所有并行任务都标记为完成
                    start_bs = task_idx * group_n
                    end_bs = start_bs + group_n
                    for g_bs in range(start_bs, min(end_bs, batch_size)):
                        if not is_done[g_bs]:
        ...
        for bs in range(batch_size):
            if turn_in_range[bs]:
                task_idx = bs // group_n
                status_msgs[bs] = f"Task {task_idx} out of max turn"
                print(status_msgs[bs])
    ...
    def multi_turn_loop(
        ...
        """
        if is_train:
            # 标记 此处重复self.config.env.rollout.n即group_n次
            # 当train_batch_size=1，self.config.env.rollout.n=5时，
            # 原始的1个任务（同一个goal）会被重复5次，创建5个独立的任务条目，正好分配给5个worker。
