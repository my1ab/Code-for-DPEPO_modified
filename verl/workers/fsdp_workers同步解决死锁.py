...
class ActorRolloutRefWorker(Worker):
    ...
    def _build_model_optimizer(
        ...
        if role == "actor" and optim_config is not None:
            ...
            # 标记 将创建好的优化器和学习率调度器赋值给实例变量，供save_checkpoint使用
    ...
    def save_checkpoint(self, local_path, hdfs_path=None, global_step=0, max_ckpt_to_keep=None):
        ...
        if self._is_offload_param:
            load_fsdp_model_to_gpu(self.actor_module_fsdp)
        # breakpoint() 
        # 标记 恢复调用FSDPCheckpointManager的save_checkpoint，保存model/optim/extra_state所有文件
        ...
        if self._is_lora and isinstance(self.actor_module, PeftModel):
            # 只有当checkpoint_manager没有处理LoRA保存时，才需要手动保存
            # 检查checkpoint_manager是否已经处理了LoRA保存，避免重复保存导致的死锁
            lora_already_saved = False
            if dist.get_rank() == 0:
                lora_save_path = os.path.join(local_path, "lora_adapter")
                lora_already_saved = os.path.exists(os.path.join(lora_save_path, "adapter_model.safetensors"))
            
            # 先同步所有进程，获取是否已经保存的状态
            # 确保tensor创建在正确的分布式设备上
            device = torch.device(f'cuda:{self.rank}')
            lora_already_saved = torch.tensor(lora_already_saved, device=device)
            dist.broadcast(lora_already_saved, 0)
            lora_already_saved = lora_already_saved.item()
            
            if not lora_already_saved:
                lora_save_path = os.path.join(local_path, "lora_adapter")
                peft_config = {}
                if dist.get_rank() == 0:
                ...
                try:
                    need_manual_collect = isinstance(self.actor_module_fsdp, FSDP)
                    # 先让所有进程都知道是否需要手动收集参数
                    need_manual_collect_tensor = torch.tensor(need_manual_collect, device=device)
                    dist.broadcast(need_manual_collect_tensor, 0)
                    need_manual_collect = need_manual_collect_tensor.item()
                    
                    if need_manual_collect:
            ...
            dist.barrier()
            if dist.get_rank() == 0:
                print(f"[rank-{self.rank}]: Saved LoRA adapter to: {os.path.join(local_path, 'lora_adapter')}")
