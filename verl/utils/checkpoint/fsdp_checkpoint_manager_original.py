...
class FSDPCheckpointManager(BaseCheckpointManager):
    ...
    def load_checkpoint(self, local_path: str, hdfs_path: str = None, del_local_after_load=False):
        ...
        # 标记 检查是否存在LoRA适配器目录，如果存在则是LoRA模式，只加载LoRA权重
        lora_dir = os.path.join(local_path, "lora_adapter")
        if os.path.exists(lora_dir):
            print(f"[rank-{self.rank}]: Detected LoRA checkpoint, loading LoRA weights from {lora_dir}")
            from peft import PeftModel
            # 基础模型已经初始化，直接加载LoRA适配器
            self.model = PeftModel.from_pretrained(self.model, lora_dir)
            # 加载优化器和其他状态（这些还是保存为标准格式）
            ...
            model_state_dict = None  # LoRA模式不需要加载全量模型state_dict
        else:
            # 传统全参数训练模式，加载完整的模型文件
        ...
        with get_fsdp_state_ctx(self.model, StateDictType.SHARDED_STATE_DICT, state_dict_cfg, optim_cfg):
            # LoRA模式下已经通过PeftModel加载了适配器，不需要再load_state_dict全量模型
    ...
    def save_checkpoint(self, local_path: str, hdfs_path: str = None, global_step: int = 0, max_ckpt_to_keep=None):
        ...
        with warnings.catch_warnings():
            ...
            with get_fsdp_state_ctx(self.model, StateDictType.SHARDED_STATE_DICT, state_dict_cfg, optim_cfg):
                ...
                # 标记  保存model optim等  在verl/workers/fsdp_workers.py修复了
                print(f"[rank-{self.rank}]: Saving model to {os.path.abspath(model_path)}")
                torch.save(model_state_dict, model_path)
                # 只有当optimizer_state_dict存在时才保存优化器文件
        ...
        if self.rank == 0:
            ...
            # 检查是否是LoRA模型，如果是，直接保存LoRA适配器
            from peft import PeftModel
            if isinstance(unwrap_model, PeftModel):
                print(f"[rank-{self.rank}]: Saving LoRA adapter to {os.path.join(local_path, 'lora_adapter')}")
                lora_save_path = os.path.join(local_path, "lora_adapter")
                os.makedirs(lora_save_path, exist_ok=True)
                unwrap_model.save_pretrained(lora_save_path)
                # 同时保存processor到主目录
                self.processing_class.save_pretrained(local_path)
                # LoRA模式下仍然需要保存优化器和extra_state，前面的代码已经执行了torch.save(model_state_dict, model_path)等
            else:
                # 传统全参数模型的保存逻辑
