...
def main(config):
    ...
    print(f"[TIMING] main() started at: {time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(start_time))}")
    run_ppo(config)
    end_time = time.time()
    elapsed = end_time - start_time
    print(f"[TIMING] main() ended at: {time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(end_time))}")
    print(f"[TIMING] main() total elapsed: {elapsed:.2f}s ({elapsed/60:.2f}min)")
...
class TaskRunner:
    def run(self, config):
        ...
        print(f"[DEBUG] Created parallel environments: {config.env.num_parallel} completed in build_parallel_webshop_envs")
        print(f"[DEBUG] lazy_envs = {lazy_envs}")
...
def create_rl_dataset(data_paths, data_config, tokenizer, processor):
    """Create a dataset for RL training.

    Arguments:
        data_paths: Path(s) to the data files.
