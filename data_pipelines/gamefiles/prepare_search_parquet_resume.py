...
采样方式（源自 coldstart_search_local_3epoch.py 的 SearchTaskSampler）:
  ...
  - 排除逻辑: success_indices_merged_500_search.txt 中的 logical_idx
    ...
    [resume 版本] exclude_indices 设为空表，等效关闭排除功能。
  - [resume 版本] 训练数据采样 logical_idx 区间 [START_IDX, START_IDX + TRAIN_SIZE)

输出约定 (与 WebShop 版本一致):
  - gamefiles/search/  -> 只存储 index JSON 文件 (env.env_path 用)
  - verl_train_data/search/ -> 只存储 parquet 数据文件
  - 所有输出文件名尾缀 _resume 用于区分
...
START_IDX = 1000          # [resume] 起始 logical_idx (采样 [1000, 1500) 区间)
...
def write(shuffle_table, exclude_indices, train_goal_idxs, df_train,
    ...
    # [resume] 从 START_IDX 开始取 TRAIN_SIZE 个位置 (logical_idx 1000-1499)
    selected_positions = selected[START_IDX:START_IDX + TRAIN_SIZE]
    print(f"[{tag}] Taking positions [{START_IDX}, {START_IDX + TRAIN_SIZE}) as train set")
...
def main():
    ...
    # ── 1. 加载排除列表 ──────────────────────────────────────
    # [resume] 设为空表以关闭排除功能（等效于不排除任何 logical_idx）
    exclude_indices = set()
    print(f"[Exclude] {len(exclude_indices)} indices to exclude from train pool (resume: empty set)")
    print('=' * 80)
    ...
    # ── 9. 生成训练数据 (logical_idx 1000-1499, exclude=空表) ─
    print('=' * 80)
    print(f'>>> Generating resume version (logical_idx [{START_IDX}, {START_IDX + TRAIN_SIZE}))')
    print('=' * 80)
    ...
    test_parquet_path = os.path.join(PARQUET_DIR, 'search_test_resume.parquet')
    df_v.to_parquet(test_parquet_path, index=False)

    test_task_dict = build_task_json(test_samples, 'test', INCLUDE_EXTRA_INFO)
    test_json_path = os.path.join(JSON_DIR, 'search_test_tasks_resume.json')
    ...
    print("  Train:")
    print("    data.train_files=" + PARQUET_DIR + "/search_train_resume.parquet")
    print("    env.env_path=" + JSON_DIR + "/search_train_tasks_resume.json")
    print("  Test:")
    print("    data.val_files=" + PARQUET_DIR + "/search_test_resume.parquet")
    print("=" * 80)
