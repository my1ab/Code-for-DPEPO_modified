...
REMOTE_URL="git@github.com:my1ab/Code-for-DPEPO_dataset_extended.git"
...
EXCLUDE_PATHS=(
    ...
    3emb_model_bs4_search
    3emb_model_resume
    3emb_model_bs1/search_emb_0.8_bs1_test
    3emb_model_bs1_webshop/webshop_emb_0.8_bs1_test
    交接文档
    解析文件
    修改日志
    ...
    coldstart_genaration_search/retrieval_server.log
    eval_search/*/*/
    eval_webshop/*/*/
    emb_models
    ...
    test_bert_similarity/emb_models
    "*.pt"
    "*.ckpt"
    "*.safetensors"
    "*.tar.gz"
...
# ===== 文件大小阈值配置(单位: MiB, 脚本内自动换算为字节) =====
HIST_MAX_MIB=100   # 未推送历史提交中的大文件阈值(GitHub 单文件硬限制为 100MB, 不建议设更大)
STAGE_MAX_MIB=20   # 本次暂存区大文件阈值(超过则立即中止提交)
...
#     # 扫描未推送提交中是否有超过 ${HIST_MAX_MIB}MiB 的文件(HIST_MAX_MIB 换算为字节)
#     BAD_LARGE=$(git rev-list --objects "$REMOTE_COMMIT"..HEAD 2>/dev/null \
#         | git cat-file --batch-check='%(objecttype) %(objectsize) %(rest)' \
#         | awk -v max="$((HIST_MAX_MIB * 1024 * 1024))" '$1=="blob" && $2 > max {print $3}')
...
#         [ -n "$BAD_LARGE" ] && echo "⚠️  检测到超过 ${HIST_MAX_MIB}MiB 的大文件：" && echo "$BAD_LARGE" | head -10
...
#             fi
#             # 兜底：清理所有超过 ${HIST_MAX_MIB}MiB 的 blob
#             git filter-repo --strip-blobs-bigger-than ${HIST_MAX_MIB}M --force 2>&1
#             rm -f .git/filter-repo/already_ran
...
echo ""
echo "=== 验证暂存区中是否有超过 ${STAGE_MAX_MIB}MiB 的大文件（如有则立即中止提交）==="
LARGE_FILES=$(git diff --cached --name-only --diff-filter=ACMR | while read f; do
    [ -f "$f" ] || continue
    size=$(stat -c%s "$f" 2>/dev/null || stat -f%z "$f" 2>/dev/null || echo 0)
    if [ "$size" -gt $((STAGE_MAX_MIB * 1024 * 1024)) ] 2>/dev/null; then
        printf "%d MiB\t%s\n" "$((size/1048576))" "$f"
    fi
done)
if [ -n "$LARGE_FILES" ]; then
    echo "❌ 错误：暂存区中存在超过 ${STAGE_MAX_MIB}MiB 的文件，中止提交！"
    echo ""
    echo "$LARGE_FILES"
    echo ""
