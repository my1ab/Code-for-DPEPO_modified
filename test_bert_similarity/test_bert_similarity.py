...
# 任务类型：'webshop' -> click 精确匹配 + search 词集匹配+嵌入兜底（方案 B+E）
# 对应 EmbeddingManager 新 emb 定义中的 task_type 参数
TASK_TYPE = "webshop"
# 推理设备：强制 CPU（绕过 Ray actor GPU 可见性问题，bge-large 是小模型 CPU 足够）
...
def test_webshop_pairs(emb_mgr: EmbeddingManager, threshold: float = 0.975):
    """测试 WebShop 动作对的相似度（方案 B：click 精确匹配 + search 嵌入阈值）。

    Args:
        emb_mgr: EmbeddingManager 实例（仅 search 动作使用）
        threshold: search 嵌入相似度阈值（click 不受此阈值影响，走精确匹配）
    """
    print("\n" + "=" * 70)
    print("测试 4: WebShop 动作相似度验证（方案 B+E: click精确 + search词集匹配+嵌入兜底）")
    print(f"  search 阈值: {threshold}  |  click: 精确匹配(lower)  |  词集匹配: 优先  |  task_type={emb_mgr.task_type}")
    print("=" * 70)
    ...
    print(f"\n  {'#':<3} {'动作A':<28} {'动作B':<28} {'原始':>7} {'归一':>7}  {'判定':>2}  {'期望':>2}  类别")
    print("  " + "-" * 100)
    ...
    for idx, (action_a, action_b, should_match, category) in enumerate(WEBSHOP_PAIRS):
        ...
        # actual_match = sim > SIMILARITY_THRESHOLD
        ...
        print(f"  {idx:<3d} {action_a:<28s} {action_b:<28s} {raw_sim:7.4f} {sim:7.4f}  {match_str:>2s}  {expect_str:>2s}  {correct_str:>2s}  {category} [{note}]")
    ...
    if threshold_errors:
        ...
        for a, b, sim, raw_sim, actual, expected, cat in threshold_errors:
            ...
            print(f"    [{tag}] {a:<28s} vs {b:<28s} raw={raw_sim:.4f} norm={sim:.4f} [{cat}]")
...
def test_webshop_matrix(emb_mgr: EmbeddingManager):
    """构建 WebShop 动作的相似度矩阵。"""
    print("\n" + "=" * 70)
...
def main():
    print("=" * 70)
    print("BERT 嵌入模型相似度测试（基于 EmbeddingManager）- WebShop 任务版")
    print(f"模型目录: {MODEL_DIR}")
    print(f"相似度阈值: {SIMILARITY_THRESHOLD}")
    print(f"任务类型: {TASK_TYPE}")
    print(f"推理设备: {EMBEDDING_DEVICE}")
    print("=" * 70)

    # 加载 EmbeddingManager（CPU 推理，绕过 GPU 问题）
    # 使用新 emb 定义：通过 task_type='webshop' 显式启用
    #   click 精确匹配 + search 词集匹配+嵌入兜底（方案 B+E）
    print(f"\n[1/1] 正在加载 EmbeddingManager (task_type={TASK_TYPE}, device={EMBEDDING_DEVICE}) ...")
    ...
    test_webshop_pairs(emb_mgr, threshold=SIMILARITY_THRESHOLD)
    test_webshop_matrix(emb_mgr)
