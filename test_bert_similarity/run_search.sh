...
threshold=0.8
LOG_FILE="$SCRIPT_DIR/log/test_bert_search_${threshold}_$(date +%Y%m%d_%H%M%S).log"
...
"$PYTHON" "$SCRIPT_DIR/test_bert_similarity_search.py" \
    --threshold $threshold 2>&1 | tee "$LOG_FILE"
